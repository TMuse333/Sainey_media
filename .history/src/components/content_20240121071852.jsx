...
const Content = ({ title, description, rev, link, linkText, hasTitle, id, hasAnimation, hasTilt }) => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // Gradually increase the tilt angle from 0 to 20 as the bottom 40% of the element becomes visible
            const newTiltAngleBottom = (visiblePercentBottom / 40) * 20;
    
            // Determine whether the top half of the element is not in the viewport
            const topHalfNotInView = visiblePercentTop >= 10;
    
            // Set the tilt angle based on the conditions
            setTiltAngle(topHalfNotInView ? newTiltAngleBottom : newTiltAngleTop);
