...
const Content = ({ title, description, rev, link, linkText, hasTitle, id, hasAnimation, hasTilt }) => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            const newTiltAngleBottom = (visiblePercentBottom / 60) * 40;
    
            // Start bottom tilting only when the top half of the element is not in the viewport
            if (visiblePercentTop < 200) {
                setTiltAngle(newTiltAngleBottom);
            } else {
                setTiltAngle(newTiltAngleTop);
            }
